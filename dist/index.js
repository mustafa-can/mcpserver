...
async function detectProjectStack(projectRoot) {
    try {
...
async function handleTestManagement(projectRoot, action) {
    ...
    if (action === "run") {
        ...
        if (stack.language === "Node.js") {
            ...
            try {
                ...
                if (subFiles.some(f => f.includes('playwright') || f.includes('test') || f.includes('spec')))
                    testsFound = true;
            }
            catch { }
        }
